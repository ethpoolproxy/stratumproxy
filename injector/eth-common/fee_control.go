			...
			for {
				...
				case <-time.After(2 * time.Second):
					...
					if !m.IsOnline() || !feeWorkerMiners.HasMiner(m) {
						logrus.Debugf("[%s][%s][%s][%f] 矿机 [%s] 在抽水的时掉线，取消本次抽水！", feeInfo.PoolServer.Config.Name, feeInfo.Wallet, feeInfo.NamePrefix, feeInfo.Pct, m.GetID())
						return
					}
					if feeInfo.UpstreamClient.Ctx.Err() != nil {
						logrus.Debugf("[%s][%s][%s][%f] 矿机 [%s] 抽水上游正在重连或者已断线，取消本次抽水！", feeInfo.PoolServer.Config.Name, feeInfo.Wallet, feeInfo.NamePrefix, feeInfo.Pct, m.GetID())
						return
					}
